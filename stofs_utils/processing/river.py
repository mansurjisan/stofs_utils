...
def generate_flux_timeseries(date, rivers, river_files, output_file='flux.th'):
    ...
    This function reads discharge data for specified rivers, calculates
    time relative to the start date, and writes the data to a SCHISM-compatible
    flux.th file.

    Note: Flow values in the output file are negated (-flow), following
          the SCHISM convention where negative values represent sources (inflow).
    ...
    rivers : list
        List of river names corresponding to columns in the output file.
    river_files : dict
        Dictionary mapping river names (from `rivers` list) to their
        corresponding discharge data CSV files.
    ...
    -------
    str
        Path to the generated output file (flux.th).
    """
...
def get_river_temperature(date, air_temp_file, rivers, output_file='TEM_1.th'):
    ...
    idxs = waterT < DEFAULT_TEMP_THRESHOLD
    waterT[idxs] = DEFAULT_TEMP_THRESHOLD
...
def generate_river_input(date, config, output_dir='.', ntracers=DEFAULT_TRACERS):
    """
    ...
    ntracers : int, optional
        Number of tracers (default: DEFAULT_TRACERS)
