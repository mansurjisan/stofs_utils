...
class Hotstart:
    ...
    def interp_from_existing_hotstart(self, hot_in, iplot=False, i_vert_interp=True):
        ...
        # Find nearest neighbors in horizontal dimension
        from ..core.coordinate_utils import near_pts
        ...
        if iplot:
            ...
            plt.scatter(grid_in.hgrid.side_x, grid_in.hgrid.side_y, s=5, 
                       c=hot_in.su2.val[:, plot_layer], cmap='jet', vmin=0, vmax=2)
...
def gather_grid_info(grid_in, eta, dir):
    ...
    if not hasattr(grid_in, 'hgrid'):
        from ..io.grid_io import read_schism_hgrid
    ...
    if not hasattr(grid_in, 'vgrid'):
        from ..io.grid_io import read_schism_vgrid
